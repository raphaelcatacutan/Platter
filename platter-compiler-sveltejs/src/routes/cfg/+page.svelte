...
<svelte:window on:load={initParser} />
...
<div class="sidebar" id="sidebar">
	...
	<div class="controls">
		<button on:click={runParser}>Parse & Visualize</button>
		<button class="secondary" on:click={toggleTable}>Show Table</button>
	</div>
	
	<div class="controls">
		<button class="secondary" on:click={visualizeParseTree}>Visualize Parse Tree</button>
	</div>
...
<div id="tableModal">
	<div class="modal-header">
		...
		<button on:click={toggleTable} style="width: auto; padding: 2px 10px;">Close</button>
	</div>
...
<script lang="ts">
	...
	// --- PARSER STATE ---
	let grammar: Record<string, string[][]> = {};
	let terminals = new Set<string>();
	let nonTerminals = new Set<string>();
	let startSymbol: string | null = null;
	let parseTable: Record<string, Record<string, string[]>> = {};
	let firstSets: Record<string, Set<string>> = {};
	let followSets: Record<string, Set<string>> = {};
	...
	function runParser() {
		...
		localStorage.setItem('cfg_input', (document.getElementById('cfgInput') as HTMLTextAreaElement).value);
		localStorage.setItem('test_input', (document.getElementById('testInput') as HTMLInputElement).value);
		...
		try {
			...
			// Run Trace
			const inputStr = (document.getElementById('testInput') as HTMLInputElement).value;
			runTrace(inputStr);
	...
	function parseGrammar() {
		const text = (document.getElementById('cfgInput') as HTMLTextAreaElement).value;
		grammar = {};
		terminals = new Set();
	...
	function getFirstSeq(seq: string[]) {
	...
	function runTrace(inputRaw: string) {
		const log = document.getElementById('logPanel')!;
	...
	function appendLog(s: string, i: string, a: string, success=false, err=false, isHeader=false) {
		const log = document.getElementById('logPanel')!;
	...
	function appendInfo(msg: string) {
		const log = document.getElementById('logPanel')!;
		const d = document.createElement('div');
	...
	function createParseTreeNode(node: any) {
	...
	function renderTree(rootToken: string) {
		const root = document.getElementById('treeRoot')!;
		root.innerHTML = '';
		if(!rootToken) return;
		const ul = document.createElement('ul');
	...
	function createNode(token: string, isOpt=false) {
		const li = document.createElement('li');
		const div = document.createElement('div');
	...
	function expandNode(li: HTMLElement, token: string) {
	...
	function fillRow(rhs: string[], container: HTMLElement) {
	...
	function resetZoom() {
		...
		st.style.transform=`translate(-50%, 0) scale(1)`;
	}
	...
	function initParser() {
		...
		const st = document.getElementById('stage')!;
		vp.onmousedown = (e: any) => { 
			if(e.target===vp||e.target===st){
				e.preventDefault(); 
				isDrag=true; 
				sx=e.clientX-px; 
				sy=e.clientY-py;
		...
		window.onmousemove = (e) => { 
			if(isDrag){ 
				...
				px=e.clientX-sx; 
				py=e.clientY-sy; 
				st.style.transform=`translate(calc(-50% + ${px}px), ${py}px) scale(${scale})`; 
			} 
		};
		vp.onwheel = (e) => { 
			e.preventDefault(); 
		...
		if (savedCfg) {
			(document.getElementById('cfgInput') as HTMLTextAreaElement).value = savedCfg;
		}
		if (savedTest) {
			(document.getElementById('testInput') as HTMLInputElement).value = savedTest;
		}
